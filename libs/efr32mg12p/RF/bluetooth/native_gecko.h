...
* Set the Bluetooth public address used by the device. A valid address
* set with this command overrides the default Bluetooth public address
* programmed at production and is effective in the next system reboot.
* The stack treats 00:00:00:00:00:00 and ff:ff:ff:ff:ff:ff as invalid
* addresses. As a result, passing one of them into this command will
* cause the stack to use the default address in the next system reboot.
*  
*
...
* device does not meet the power requirements. For Bluetooth
* connections, the maximum TX power is limited to 10 dBm if Adaptive
* Frequency Hopping (AFH) is not enabled.
...
* Use this command only for a short time period (a few seconds at
* maximum).                 Although it halts Bluetooth activity, all
* tasks and operations still exist inside the                 stack with
* their own concepts of time. Halting the system for a long time period
* may                 have negative consequences on stack's internal
* states.
...
* @param data_len   Array length
* @param data_data   Configuration data. Length and contents of the data field depend on the key value used.
*
**/
...
* The new address will be effective in the next system reboot. The stack
* will use the address in the PS keys when present. Otherwise, it uses
* the default Bluetooth public device address which is programmed at
* production.
* The stack treats 00:00:00:00:00:00 and ff:ff:ff:ff:ff:ff as invalid
* addresses. Therefore, passing one of them into this command will cause
* the stack to delete the PS keys and use the default address in the
* next system reboot.
...
* Events generated
*
* gecko_evt_le_connection_opened - Triggered after the connection is opened and indicates whether the devices are already bonded and whether the role of the Bluetooth device is Slave or Master.
...
* Events generated
*
* gecko_evt_le_gap_adv_timeout - Triggered when the number of advertising events is done and advertising has stopped.
...
* Events generated
*
* gecko_evt_le_gap_scan_response - Each time an advertising packet is received, this event is triggered. The packets are not filtered in any way, so multiple events will be                     received for every advertising device in range.
*
**/
BGLIB_DEPRECATED_API 
...
* Set the default Bluetooth connection parameters. The configured values
* are valid for all subsequent connections that will             be
* established. To change the parameters of an already established
* connection, use the command "le_connection_set_parameters".
*  
*
* @param min_interval   Minimum value for the connection event interval. This must be set less than or equal to the max_interval.
*  - Time = Value x 1.25 ms
*  - Range: 0x0006 to 0x0c80
*  - Time Range: 7.5 ms to 4 s
* Default value: 20 ms                 
* @param max_interval   Maximum value for the connection event interval. This must be set greater than or equal to the min_interval.
*  - Time = Value x 1.25 ms
*  - Range: 0x0006 to 0x0c80
*  - Time Range: 7.5 ms to 4 s
* Default value: 50 ms                 
* @param latency   Slave latency, which defines how many connection intervals the slave can skip if it has no data to send
*  - Range: 0x0000 to 0x01f4
* Default value: 0                 
* @param timeout   Supervision timeout, which defines the time that the connection is maintained although the devices can't communicate at the currently configured connection intervals.
...
* The parameters set by this command are only effective on the 1M PHY.
* For Coded PHY, use the above replacement command.
*  
*
* @param scan_interval   Scanner interval is defined as the time interval when the device starts its last scan until it begins the subsequent scan. In other words, it indicates how often to scan
*  - Time = Value x 0.625 ms
*  - Range: 0x0004 to 0x4000
*  - Time Range: 2.5 ms to 10.24 s
* Default value: 10 ms 
* A variable delay occurs when switching channels at the end of each
* scanning interval, which is included in the scanning interval time.
* During the switch time no advertising packets are received by the
* device. The switch time variation is use case-dependent. For example,
* if scanning while keeping active connections, the channel switch time
* might be longer than scanning without any active connections.
* Increasing the scanning interval reduces the amount of time in which
* the device can't receive advertising packets because it will switch
* channels less often.
* After every scan interval, the scanner changes the frequency at which
* it operates. It cycles through all three advertising channels in a
* round robin fashion. According to the specification, all three
* channels must be used by the scanner.
* 
* @param scan_window   Scan window defines the duration of the scan which must be less than or equal to scan_interval
*  - Time = Value x 0.625 ms
*  - Range: 0x0004 to 0x4000
*  - Time Range: 2.5 ms to 10.24 s
...
*  
*
* @param scan_rsp   This value selects if data is intended for advertising packets, scan response packets, or advertising packet in OTA. Values: 
...
* The parameter accepted_phy is used to specify PHYs that the stack can
* accept in a remotely-initiated PHY update request.             A PHY
* update will not happen if none of the accepted PHYs are present in the
* request.
...
* @param accepted_phy   Accepted PHYs in remotely-initiated PHY update request. This parameter is a bitfield and multiple PHYs can be set.                         
*  - 0x01: 1M PHY
*  - 0x02: 2M PHY
*  - 0x04: Coded PHY
*  - 0xff: Any PHYs
...
* @param discover   Discoverable mode
* @param connect   Connectable mode
* @param maxevents   If non-zero, indicates the maximum number of advertising events to send before stopping the advertiser. Value 0 indicates no maximum number limit.
* @param address_type   Address type to use for packets
*
* Events generated
*
* gecko_evt_le_gap_adv_timeout - Triggered when the advertising events set by this command are complete and advertising is stopped on the given advertising set.
* gecko_evt_le_connection_opened - Triggered when a remote device opens a connection to the advertiser on the specified advertising set.
*
**/
BGLIB_DEPRECATED_API 
...
* Events generated
*
* gecko_evt_le_gap_scan_request - Triggered when a scan request is received during advertising if the scan request notification is enabled by this command.
*
**/
BGLIB_DEPRECATED_API 
...
* If advertising mode is currently enabled, the new advertising data
* will be used immediately.             Advertising mode can be enabled
* using command             "le_gap_start_advertising".
* Periodic advertising mode can be enabled using command
* "le_gap_start_periodic_advertising".
* The invalid parameter error will be returned in the following
* situations:
...
* the advertising is later enabled in a discoverable mode other than
* user_data.
*  
*
* @param handle   Advertising set handle
* @param scan_rsp   This value selects whether data is intended for advertising packets, scan response packets, periodic advertising packets, or advertising packets in OTA. Values are as follows:                 
*  - 0: Advertising packets
*  - 1: Scan response packets
*  - 2: OTA advertising packets
*  - 4: OTA scan response packets
...
*  - 1: Enable privacy
* 
* @param interval   The minimum time interval between a private address change. This parameter is ignored if this command is issued to disable privacy mode. Values: 
...
*  - Note: interval_max should be bigger than interval_min
* Default value: 200 ms
* @param duration   Advertising duration for this advertising set. Value 0 indicates no advertising duration limit and advertising continues until it is disabled. A non-zero value sets the duration in units of 10 ms. The duration begins at the start of the first advertising event of this advertising set.
*  - Range: 0x0001 to 0xFFFF
*  - Time range: 10 ms to 655.35 s
* Default value: 0
* @param maxevents   If non-zero, indicates the maximum number of advertising events to send before the advertiser is stopped. Value 0 indicates no maximum number limit. 
* Default value: 0
* 
*
**/
...
*
* @param handle   Advertising set handle
* @param primary_phy   The PHY on which the advertising packets are transmitted on the primary advertising channel. If legacy advertising PDUs are used, 1M PHY must be used.
* Values:
...
*
* Enable advertising configuration flags on the given advertising set.
* The                 configuration change will take effect next time
* that advertising is enabled.
...
*
* Disable advertising configuration flags on the given advertising set.
* The                 configuration change will take effect next time
* that advertising is enabled.
...
* extended advertising PDUs if either of the following has occurred with
* the default configuration:
*  - The connectable mode is set to le_gap_connectable_non_scannable.
*  - The primary advertising PHY is set to Coded PHY by the command "le_gap_set_advertise_phy".
...
* Specifically, this command fails with  the connection limit exceeded
* error if it causes the number of connections exceeding the configured
* MAX_CONNECTIONS value. It fails with the invalid parameter error if
* one of the following use cases occurs:
...
*  - le_gap_connectable_non_scannable is the connectable mode but using legacy advertising PDUs has been explicitly enabled with command "le_gap_set_advertise_configuration".
*  - Coded PHY is the primary advertising PHY but using legacy advertising PDUs has been explicitly enabled with command "le_gap_set_advertise_configuration".
*  - le_gap_connectable_scannable is the connectable mode but using extended advertising PDUs has been explicitly enabled or the primary advertising PHY is set to Coded PHY.
* If advertising is enabled in user_data mode, use
* "le_gap_bt5_set_adv_data" to set advertising and scan response data
* before issuing this command. When advertising is enabled in modes
* other than user_data, advertising and scan response data is generated
* by the stack using the following procedure:
...
* Event "le_gap_adv_timeout" will be received when the number of
* advertising events set by "le_gap_set_advertise_timing" command is
* done and advertising with the current set has stopped.
*  
*
* @param handle   Advertising set handle
* @param discover   Discoverable mode
...
* Events generated
*
* gecko_evt_le_gap_adv_timeout - Triggered when the number of advertising events set by "le_gap_set_advertise_timing" command is done and advertising has stopped on the given advertising set.
* gecko_evt_le_connection_opened - Triggered when a remote device opens a connection to the advertiser on the specified advertising set and also advertising with the current set stops.
*
**/
...
* 
* @param scan_interval   Scan interval is defined as the time interval when the device starts its last scan until it begins the subsequent scan. In other words, how often to scan
*  - Time = Value x 0.625 ms
...
* A variable delay occurs when switching channels at the end of each
* scanning interval, which is included in the scanning interval time.
* During the switch time, advertising packets are not received by the
* device. The switch time variation is use case-dependent. For example,
* if scanning while keeping active connections, the channel switch time
* might be longer than when scanning without any active connections.
* Increasing the scanning interval reduces the amount of time in which
* the device can't receive advertising packets because it switches
* channels less often.
* After every scan interval, the scanner changes the frequency at which
* it operates. It cycles through all three advertising channels in a
* round robin fashion. According to the specification, all three
...
* 
* @param scan_window   Scan window defines the duration of the scan which must be less than or equal to the scan_interval
*  - Time = Value x 0.625 ms
*  - Range: 0x0004 to 0xFFFF
*  - Time Range: 2.5 ms to 40.96 s
...
*
* Start the GAP discovery procedure to scan for advertising devices on
* the specified scanning PHY or to perform a device discovery. To cancel
* an ongoing             discovery process use the
* "le_gap_end_procedure" command.
* The invalid parameter error will be returned if the scanning PHY value
...
* Events generated
*
* gecko_evt_le_gap_scan_response - This event is triggered each time an advertising packet is received. Packets are not filtered in any way, so multiple events will be                     received for every advertising device in range.
* gecko_evt_le_gap_extended_scan_response - Each time an advertising packet is received and extended scan response is enabled (by "le_gap_set_discovery_extended_scan_response"), this event is triggered. The packets are not filtered in any way. As a result, multiple events will be                     received for every advertising device in range.
*
**/
...
* @param channel_map_data   This parameter is 5 bytes and contains 37 1-bit fields.{br}                 The nth such field (in the range 0 to 36) contains the value for the link layer channel index n.{br}                 
*  - 0: Channel n is bad.
*  - 1:  Channel n is unknown.
* The rest of most significant bits are reserved for future use and shall be set to 0.{br}                 At least two channels shall be marked as unknown.                 
*
**/
...
*
* Connect to an advertising device with the specified initiating PHY on
* which                 connectable advertisements on primary
* advertising channels are received.                 The Bluetooth stack
* will enter a state where it continuously                 scans for the
* connectable advertising packets from the remote
...
* If a connection can't be established (for example,                 the
* remote device has gone out of range, has entered into deep sleep, or
* is not advertising), the stack will try to connect forever.
* In this case, the application will not get an event related to the
* connection request.                 To recover from this situation,
* the application can implement a timeout and call
* "le_connection_close" to cancel the connection request.
* This command fails with the connection limit exceeded error if
* the number of connections attempted exceeds the
* configured MAX_CONNECTIONS value.
* This command fails with the invalid parameter error if the initiating
* PHY value is invalid or the device does not support PHY.
* Later calls of this command have to wait for the ongoing command
* to complete. A received event                  "le_connection_opened"
* indicates that the connection opened successfully and a received event
* "le_connection_closed"                 indicates that connection
...
* Events generated
*
* gecko_evt_le_connection_opened - This event is triggered after the connection is opened and indicates whether the devices are already bonded and whether the role of the Bluetooth device is Slave or Master.
...
* The invalid parameter error is returned if the application has
* configured                 legacy advertising PDUs or anonymous
* advertising, or the advertising set is                 enabled using
...
* i.e., legacy or extended advertising is not stopped.
*  
*
* @param handle   Advertising set handle
*
...
*
* Set advertising data for a specified packet type and advertising set.
* Data currently in the system data buffer will be extracted as the
* advertising data. The buffer             will be emptied after this
* command regardless of the completion status.
* Prior to calling this command, add data to the buffer with one or
* multiple calls of             "system_data_buffer_write".
* Maximum 31 bytes of data can be set for legacy advertising. Maximum
* 191 bytes of data can be set for connectable extended advertising.
* Maximum 1650 bytes of data can be set for periodic and non-connectable
* extended advertising,             but advertising parameters may limit
* the amount of data that can be sent in a single advertisement.
* See "le_gap_bt5_set_adv_data" for more details on advertising data.
*  
*
* @param handle   Advertising set handle
* @param packet_type   This value selects whether data is intended for advertising packets, scan response packets, or periodic advertising packets. Values:                     
*  - 0: Advertising packets
*  - 1: Scan response packets
*  - 2: OTA advertising packets
*  - 4: OTA scan response packets
...
*
* @param min_interval   Minimum value for the connection event interval. This must be set less than or equal to max_interval.
*  - Time = Value x 1.25 ms
*  - Range: 0x0006 to 0x0c80
*  - Time Range: 7.5 ms to 4 s
* Default value: 20 ms                 
...
* When setting a resolvable random address, the address parameter is
* ignored. The stack generates             a private resolvable random
* address and set it as the advertiser address. The generated address is
...
* advertising set.             Invalid parameter error is returned if
* the advertising set handle is invalid or the address does not conforms
* to the Bluetooth specification.
*  
*
* @param handle   Advertising set handle
...
* 
* @param address   The random address to set. Ignore this field when setting a resolvable random address.
*
**/
...
* Wrong state error is returned if advertising has been enabled on the
* advertising set.             Invalid parameter error is returned if
* the advertising set handle is invalid.
*  
*
* @param handle   Advertising set handle
*
...
* gecko_cmd_sync_open
*
* Establish a synchronization with a periodic advertising from the
* specified             advertiser and begin receiving periodic
* advertising packets. Note that synchronization establishment can only
* occur when scanning is enabled. While scanning is disabled, no attempt
* to synchronize will occur.
...
* If skip and timeout are used, select appropriate values so that they
* allow a few receiving attempts.             Periodic advertising
* intervals are reported in event
...
* Events generated
*
* gecko_evt_sync_closed - Triggered after a periodic advertising synchronization has been closed or canceled.
*
**/
...
* Request a change in the connection parameters of a Bluetooth
* connection.
*  
*
...
* Set preferred PHYs for a connection. Preferred PHYs are connection-
* specific. Event "le_connection_phy_status" is received when PHY update
* procedure is completed. Non-preferred PHY can also be set if remote
* device does not accept any of the preferred PHYs.
* NOTE: 2 Mbit and Coded PHYs are not supported by all devices.
*  
*
* @param connection   
* @param phy   Preferred PHYs for connection. This parameter is a bitfield and multiple PHYs can be preferred by setting multiple bits.                         
*  - 0x01: 1M PHY
*  - 0x02: 2M PHY
...
*  - The value in milliseconds must be larger than (1 + latency) * max_interval * 2, where max_interval is given in milliseconds
* Set the supervision timeout at a value which allows communication attempts over at least a few connection intervals.                 
* @param min_ce_length   Minimum value for the connection event length. This must be set less than or equal to max_ce_length.
*  - Time = Value x 0.625 ms
*  - Range: 0x0000 to 0xffff
* Value is not currently used and is reserved for future. Set to 0.                 
* @param max_ce_length   Maximum value for the connection event length. This must be set greater than or equal to min_ce_length.
...
* The parameter accepted_phy is used for specifying the PHYs that the
* stack can accept in a remote initiated PHY update request.
* A PHY update will not occur if none of the accepted PHYs presents in
* the request.
* NOTE: 2M and Coded PHYs are not supported by all devices.
*  
*
...
*  - 0x08: 500k Coded PHY (S=2)
* Default: 0xff (no preference)                     
* @param accepted_phy   Accepted PHYs in remotely-initiated PHY update requests. This parameter is a bitfield and multiple PHYs can be set.                         
*  - 0x01: 1M PHY
*  - 0x02: 2M PHY
*  - 0x04: Coded PHY
*  - 0xff: Any PHYs
...
* Note that the GATT client does not verify if the requested attribute
* is a characteristic value.             Therefore, before calling this
* command, ensure that the attribute handle is for a characteristic
* value, for example, by performing characteristic discovery.
*  
*
* @param connection   
* @param characteristic   
...
*
* Add a characteristic value to the write queue of a remote GATT server.
* It can be used when long attributes need to be written or a set of
* values needs to be written atomically. At most ATT_MTU - 5 amount of
* data can be sent at one time. Writes are executed or canceled with the
...
* In all use cases where the amount of data to transfer fits into the
* BGAPI payload, use the command
* "gatt_write_characteristic_value" to write long values because it
* transparently             performs the prepare_write and execute_write
* commands.
*  
*
* @param connection   
...
*
* Read values of multiple characteristics from a remote GATT database at
* once.             The GATT server returns values in one ATT PDU as the
* response. If the total set of values is greater than
* (ATT_MTU - 1) bytes in length, only the first (ATT_MTU - 1) bytes are
* included. A single             "gatt_characteristic_value" event is
* generated, in which the             characteristic is set to 0 and
* data in the value parameter is a concatenation of characteristic
* values in the order             they were requested.             The
* received "gatt_procedure_completed" event indicates either that this
* GATT procedure             was successfully completed or failed with
* an error.
* Use this command only for characteristics values that have a known
* fixed size, except the last one that could have variable length.
* When the remote GATT server is from Silicon Labs Bluetooth stack, the
* server returns             ATT Invalid PDU (0x04) if this command only
* reads one characteristic value. The server returns             ATT
* Application Error 0x80 if this command reads the value of a user-type
* characteristic.
*  
*
* @param connection   
...
* @param characteristic   
* @param offset   Offset of the characteristic value
* @param maxlen   Maximum bytes to read. If this parameter is 0, all characteristic values starting at a given offset will be read.                     
*
* Events generated
*
* gecko_evt_gatt_characteristic_value - Contains the data of a characteristic sent by the GATT Server.
...
*
* Add a characteristic value to the write queue of a remote GATT server
* and verifies whether the value was correctly received by the server.
* Received "gatt_procedure_completed" event indicates that this GATT
* procedure was successfully completed or failed with an error.
* Specifically, error code 0x0194 (data_corrupted) will be returned if
* the value received from the GATT server's response fails to pass the
* reliable write verification. At most ATT_MTU - 5 amount of data can be
* sent at one time. Writes are executed or canceled with the
* "execute_characteristic_value_write" command.             Whether the
* writes succeed or not is indicated in the response of the
* "execute_characteristic_value_write" command.
*  
...
* @param att_errorcode   
* @param value_len   Array length
* @param value_data   Characteristic value to send to the GATT client. Ignored if att_errorcode is not 0.
*
**/
...
* Configuration descriptor. If the Client Characteristic Configuration
* descriptor supports both notifications and indications, the stack will
* always send a notification even when the client has enabled both.
...
* is completed when a confirmation from the client is received. The
* confirmation is indicated             by
...
* notification.             The same applies to the indication property,
* and in addition, bg_err_wrong_state is returned if an indication
* procedure with the same client is outstanding. Always check the
* response for this command for errors before trying to send more data.
*  
*
* @param connection   A handle of the connection over which the notification or indication is sent. Values: 
...
* Service Changed characteristic, the stack will monitor the local
* database change status and manage service changed indications for a
* GATT client that has enabled the indication configuration of the
* Service Changed characteristic.
*  
*
* @param caps   Bit flags of capabilities to reset. Value 0 sets the default database capabilities.
* @param reserved   Use the value 0 on this reserved field. Do not use none-zero values because they are reserved for future use.
*
**/
...
*
* @param time   Frequency interval of events, which indicates how often to send events in hardware clock ticks (1 second is equal to 32768 ticks).                 
...
* functionality. Call sl_sleeptimer_get_tick_count64 to get current tick
* count. Sleep Timer provides APIs for conversions between ticks and
* milliseconds.
...
* packet_type and length parameters. The parameter phy specifies which PHY is used to transmit the packets. All devices support at least 1M PHY. A special packet type, test_pkt_carrier, can be used to transmit continuous unmodulated carrier. The length field is ignored in this mode.
...
* Parameter
* phy specifies which PHY is used to receive the packets. All devices support at least 1M PHY.
...
* Events generated
*
* gecko_evt_le_connection_parameters - Triggered after increasing security has been completed successfully and indicates the latest security mode of the connection.
* gecko_evt_sm_bonded - Triggered if pairing or bonding was performed in this operation and the result is successful.
* gecko_evt_sm_bonding_failed - Triggered if pairing or bonding was performed in this operation and the result has failed.
*
**/
...
* @param oob_data_data   OOB data. To set OOB data, send a 16-byte array. To clear OOB data, send a zero-length array.
*
**/
...
*
* @param oob_data_len   Array length
* @param oob_data_data   Remote device OOB data and confirm values. To set OOB data, send a 32-byte array.                         First 16-bytes is OOB data and last 16-bytes the confirm value. To clear OOB data, send a zero-length array.
*
**/
...
*
* Configure the Apple HomeKit accessory and its settings. The configuration can be reinitialized at run time. New fast advertising parameters will be used for next fast advertising. 
*
* @param i2c_address   I2C address of Apple authentication coprocessor
* @param support_display   A flag to indicate that the display support is enabled in the accessory.                                 {br}A pin code will be randomly generated during the pairing process and "homekit_setupcode_display" event will be triggered to ensure that the pin code can be displayed.                                 
...
* @param configuration_number   Apple HomeKit configuration number                                                         {br}By default, this starts at 1. Accessories must increment this value after a firmware update. This value must be managed by the application.
* @param fast_advert_interval   Fast advertising interval. {br}The interval is used during fast advertising in disconnected state after calling command "homekit_event_notification" when broadcast events advertising is complete.
* @param fast_advert_timeout   Fast advertising timeout. {br}The timeout is used during fast advertising in disconnected state after calling command "homekit_event_notification" when broadcast events advertising is complete.                    
*  - Time = Value x 100 ms
* 
...
*  - 0x00000002: Manual set of scan response data. When enabled, use "le_gap_bt5_set_adv_data" command to set custom scan response data. Also, HomeKit library uses it to set the accessory local name.                                  - other:  Reserved. Must be 0.
* 
* @param broadcast_advert_timeout   Broadcast events advertising timeout. {br}The timeout is used during broadcast events advertising in disconnected state after calling command "homekit_event_notification"                    
*  - Time = Value x 100 ms
* 
* @param model_name_len   Array length
* @param model_name_data   Model Name characteristic value from HomeKit Accessory Information service. Mandatory for HomeKit software authentication usage.
*
**/
...
*
* Start or stop Apple HomeKit accessory advertising. The command and parameters will take effect immediately. If the given parameters can't be used in the currently active mode, an error is returned. 
*
...
*
* Delete all Apple HomeKit pairing data. Additionally, it resets all required HomeKit settings to factory state, e.g., it resets GSN value and generates a new Device ID. 
*
*
**/
...
*
* Create an I2C test connection with Apple authentication co-processor and return an error if communication fails. 
*
*
**/
...
*
* Send a response to a "homekit_write_request" event. The response needs to be sent within 30 seconds, otherwise additional GATT             transactions are not allowed by the remote side.             {br}{br}If the status_code is set to 0, the HAP will send a response informing that the write operation was processed successfully and other values will cause the HAP to send a HAP error status response. 
*
* @param connection   
* @param characteristic   
...
*
* Send a response to a "homekit_read_request" event. The response needs to be sent within 30 seconds, otherwise further GATT transactions are not allowed by the remote side.             {br}{br}If status_code is set to 0, the characteristic value is sent to the remote GATT client through HomeKit library in a standard way. Other status_code values cause a HAP error status response instead of sending data.             {br}{br}If the value data size is less than attribute_size, the Apple HomeKit library will send new "homekit_read_request" event with a suitable offset. The Apple HomeKit library provides automatic formatting for both the frame and encryption. 
*
* @param connection   
* @param characteristic   
* @param status_code   HomeKit Status Code
...
* @param value_len   Array length
* @param value_data   Characteristic value to send to the GATT client through the HomeKit library. This is ignored if status_code is not set to 0.
*
**/
...
*
* Take an appropriate action according to connection state and the originator of the change. When device is connected and a local change occurs, it sends an empty indication to the controller. When device is disconnected, it starts broadcast events advertising. After timeout, it starts fast advertising. Broadcast and fast advertising parameters are set in "homekit_configure". After fast advertising timeout, it reverts to previous advertising settings. For both states, it sets the appropriate Global State Number value according to HomeKit specification rules.              
*
...
*
* Configure the Apple HomeKit library. This is additional configuration introduced in HAP revision R15. 
*
* @param product_data_len   Array length
* @param product_data_data   Product Data characteristic value from HomeKit Accessory Information service. Mandatory (HAP revision R15 and later).
*
**/
...
* @param options   Value of options to be changed. This parameter is used together with the mask parameter.
*
**/
...
*
* @param priority   Coexistence priority threshold. Coexistence priority is toggled if priority is below this value.
* @param request   Coexistence request threshold. Coexistence request is toggled if priority is below this value.
...
* sending the request can accept, i.e., the MTU corresponds to the
* maximum SDU size.
* Range:  23 to 65533.
...
* sending the request can accept, i.e., the MPS corresponds to the
...
* @param mtu   The maximum size of payload data that the application on the device
* sending the response can accept, i.e., the MTU corresponds to the
* maximum SDU size.
* Range:  23 to 65533.
* Application needs to handle segmentation and reassembly from PDU to
* SDU.
* 
* @param mps   The maximum size of payload data that the L2CAP layer on the device
* sending the response can accept, i.e., the MPS corresponds to the
* maximum PDU payload size.
* Range:  23 to 250.
...
*
* Send LE flow control credit indicating that the channel endpoint on local device is capable of receiving more data.              
*
...
* @param cid   The CID represents the destination channel endpoint of the device sending data.
* @param data_len   Array length
* @param data_data   Data to be sent. Data length must be within the range of destination channel endpoint's MPS value.
*
**/
...
* @param switching_pattern_data   Antenna switching pattern. Antennas will be switched in this order with the antenna switch pins during CTE. If the CTE is longer than the switching pattern, the pattern starts over.
*
**/
...
* Default: 0                     
* @param switching_pattern_len   Array length
* @param switching_pattern_data   Antenna switching pattern. Antennas will be switched in this order with the antenna switch pins during CTE. If the CTE is longer than the switching pattern, the pattern starts over. Default is the empty array.
*
**/
...
*  - 2: AoD CTE with 2 us slots
* 
* @param cte_count   The number of CTEs to be transmitted in each extended advertising interval. Currently only cte_count = 1 is supported.
* @param switching_pattern_len   Array length
* @param switching_pattern_data   Antenna switching pattern. Antennas will be switched in this order with the antenna switch pins during CTE. If the CTE is longer than the switching pattern, the pattern starts over.
*
**/
...
* Default: 1                     
* @param switching_pattern_len   Array length
* @param switching_pattern_data   Antenna switching pattern. Antennas will be switched in this order with the antenna switch pins during CTE. If the CTE is longer than the switching pattern, the pattern starts over. Default: empty array
*
* Events generated
*
